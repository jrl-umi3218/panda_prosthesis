...
PandaProsthesisImpedanceTask::PandaProsthesisImpedanceTask(const mc_rbdyn::RobotFrame & frame,
                                                           double stiffness,
                                                           double weight)
: PandaProsthesisImpedanceTask(frame, frame, frame, stiffness, weight)
{
}

PandaProsthesisImpedanceTask::PandaProsthesisImpedanceTask(const mc_rbdyn::RobotFrame & controlFrame,
                                                           const mc_rbdyn::RobotFrame & measurementFrame,
                                                           const mc_rbdyn::RobotFrame & measurementSensorFrame,
                                                           double stiffness,
                                                           double weight)
: TransformTask(controlFrame, stiffness, weight), lowPass_(0.005, cutoffPeriod_), controlFrame_(controlFrame),
  measurementFrame_(measurementFrame), measurementSensorFrame_(measurementSensorFrame)
{
  const auto & robot = controlFrame.robot();
  type_ = "impedance";
  name_ = "impedance_" + robot.name() + "_" + controlFrame.name();

  if(!measurementSensorFrame.hasForceSensor())
  {
    mc_rtc::log::error_and_throw("[{}] Measurement Sensor Frame {} does not have a force sensor attached", name_,
                                 measurementSensorFrame.name());
  }
}

void PandaProsthesisImpedanceTask::update(mc_solver::QPSolver & solver)
{
  // 1. Filter the measured wrench
  auto measuredWrenchWorld = measurementSensorFrame_->position().inv().dualMul(measurementSensorFrame_->wrench());
  measuredWrench_ = measurementFrame_->position().dualMul(measuredWrenchWorld);
  lowPass_.update(measuredWrench_);
  filteredMeasuredWrench_ = lowPass_.eval();

  // 2. Compute the compliance acceleration
  sva::PTransformd T_0_s(measurementFrame_->position().rotation());
  ...
  // All quantities here are expressed in controlFrame
  const auto T_0_cf = controlFrame_->position();
  refAccel(T_0_cf * (targetAccelW_ + deltaCompAccelW_)); // represented in the surface frame
  refVelB(T_0_cf * (targetVelW_ + deltaCompVelW_)); // represented in the surface frame
  target(compliancePose()); // represented in the world frame
}

void PandaProsthesisImpedanceTask::reset()
{
...
void PandaProsthesisImpedanceTask::load(mc_solver::QPSolver & solver, const mc_rtc::Configuration & config)
{
  if(config.has("gains"))
  {
    gains_ = config("gains");
...
void PandaProsthesisImpedanceTask::addToSolver(mc_solver::QPSolver & solver)
{
...
void PandaProsthesisImpedanceTask::addToLogger(mc_rtc::Logger & logger)
{
...
void PandaProsthesisImpedanceTask::addToGUI(mc_rtc::gui::StateBuilder & gui)
{
...
static auto registered = mc_tasks::MetaTaskLoader::register_load_function(
    "panda_brace_impedance",
    [](mc_solver::QPSolver & solver, const mc_rtc::Configuration & config)
    {
      using Allocator = Eigen::aligned_allocator<mc_tasks::force::PandaProsthesisImpedanceTask>;
      ...
      const auto & frame = [&]() -> const mc_rbdyn::RobotFrame &
      {
        ...
        {
          mc_rtc::log::deprecated("PandaProsthesisImpedanceTask", "surface", "frame");
          return robot.frame(config("surface"));
        }
        return robot.frame(config("frame"));
      }();
      auto t = std::allocate_shared<mc_tasks::force::PandaProsthesisImpedanceTask>(Allocator{}, frame);
      t->reset();
