 ...
 *
 */
struct MC_TASKS_DLLAPI PandaProsthesisImpedanceTask : mc_tasks::TransformTask
{
public:
   ...
   */
  PandaProsthesisImpedanceTask(const mc_rbdyn::RobotFrame & frame, double stiffness = 5.0, double weight = 1000.0);

  /** \brief Constructor
   *
   * \param controlFrame Frame controlled by this task
   *
   * \param measurementFrame Frame used to provide force measurements
   *
   * \param stiffness Task stiffness
   *
   * \param weight Task weight
   ...
   * \throws If the frame does not have a force sensor attached to it
   */
  PandaProsthesisImpedanceTask(const mc_rbdyn::RobotFrame & controlFrame,
  ...
  {
    const auto T_0_d = sva::PTransformd{targetPoseW_.rotation()};
  ...
  {
    targetWrench(measurementFrame_->position().dualMul(wrenchW));
  }

  /*! \brief Set the target wrench in the measurement frame. */
...
protected:
  const mc_rbdyn::ConstRobotFramePtr controlFrame_; // same as frame_ from the underlying TransformTask
  const mc_rbdyn::ConstRobotFramePtr measurementFrame_;
  const mc_rbdyn::ConstRobotFramePtr measurementSensorFrame_;
