...
{
  Eigen::Vector3d halfRotationRad = mc_rtc::constants::PI / (2 * 180.) * rotation_;

  const auto & X_0_tibiaFrame = ctl.datastore().get<sva::PTransformd>("X_0_Tibia");
  auto X_0_tibiaTarget = sva::PTransformd(mc_rbdyn::rpyToMat(halfRotationRad)) * X_0_tibiaFrame;
  const auto & X_0_femurFrame = ctl.datastore().get<sva::PTransformd>("X_0_Femur");
  auto X_0_femurTarget = sva::PTransformd(mc_rbdyn::rpyToMat(-halfRotationRad)) * X_0_femurFrame;
